...
export async function addProductCart(req, res) {
    ...
    try {
        ...
        } else if (cart.products.filter(p => p.name === name)) {
...
export async function delProductCart(req, res) {
    ...
    try {
        ...
        if (cart.products.filter(p => p.name === name).length > 1) {
            await db.collection('carts').updateOne({ user: user._id, "products.name": name }, { $inc: { "products.quantity": -1 } })
            res.status(202).send("Incremented cart's product's quantity")
        } else{
            await db.collection('carts').deleteOne({ user: user._id, "products.name": name })
            res.status(202).send("product deteleted")
        }
    } catch (error) {
        res.status(500).send(error.message)
    }
